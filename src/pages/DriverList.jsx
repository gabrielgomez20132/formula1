...
function DriverList() {
  ...
  const handleDeleted = (id) => {
    console.log("Piloto eliminado:", id);
  };
    ...
    <div className="p-6 bg-white min-h-screen">
      ...
      {loading ? (
        ...
        <>
                  ...
                  <tr key={driver.id} className="hover:bg-gray-50 transition">
                    <td className="px-6 py-4 text-center">
                      <img
                        src={driver.image}
                        alt={driver.name}
                        className="w-20 h-20 object-cover rounded-lg mx-auto shadow-lg transition-transform duration-300 ease-in-out hover:scale-120"
                      />
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      {formatDate(driver.dateOfBirth) || "Desconocido"}
                    </td>
          ...
          {totalDrivers.length > count && (
            <div className="text-center mt-6">
              <button
                onClick={() => setCount(count + 15)}
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-full font-medium shadow-md transition"
              >
                Ver más pilotos
              </button>
            </div>
